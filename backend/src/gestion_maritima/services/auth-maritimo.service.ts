...
@Injectable()
export class AuthMaritimoService {
    constructor(
    ...
    async login(loginDto: LoginDto) {
        ...
        // 2. Verificar contraseña (texto plano por ahora, igual que AuthService)
        if (usuario.contrasena !== contrasena) {
            throw new UnauthorizedException('Credenciales inválidas');
        }

        // 3. Verificar Rol
        const rolesProhibidos = ['Agente de Reservas', 'Cliente', 'Trabajador Portuario'];
